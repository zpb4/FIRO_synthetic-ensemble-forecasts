#Script to process csv HEFS data from CNRFC
...
#process data across all 3 locations
locs<-c('lamc','ukac','hopc','hopcL')
locs2<-c('LAMC1','UKAC1','HOPC1','HOPC1L')
...
for(l in 1:length(locs)){
  ...
  saveRDS(hefs_raw,paste(folder,'hefs_raw.rds',sep=''))
  ...
  saveRDS(hefs_dly_mean,paste(folder,'hefs_dly_mean_12z.rds',sep=''))
  ...
  saveRDS(hefs_ens_forc,paste(folder,'hefs_ens_forc_12z.rds',sep=''))

  hefs_ens_mean_forc<-apply(hefs_ens_forc,c(2,3),mean)

  saveRDS(hefs_ens_mean_forc,paste(folder,'hefs_ens_mean_forc_12z.rds',sep=''))
}
