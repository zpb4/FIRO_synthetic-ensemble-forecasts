...
plt_ensembles<-function(nsamps,nsets,lds,site,forc,show_leads,show_xlab,show_ylab,show_yaxslab,show_legend,plt_par,plot_samp,
...
ylm<-1.5*max(synf[(idx-i):(idx+14-i),1,1] * cfs_taf)

for(i in lds){
  plot(0:14,synf[(idx-i):(idx+14-i),1,1] * cfs_taf,type='l',lwd=3,ylim=c(0,ylm),
       xlab='',ylab='Flow (TAF/day)',main='',axes=F)
  axis(2,at=seq(0,ylm,5),labels=F)
  axis(1,at=0:14,labels = F)
  box(which='plot')
  for(j in 1:ens_num){
    lines(0:14,synf[idx-i,,j] * cfs_taf,col='light gray')
  }
  if(plot_mean==T){
    lines(0:14,apply(synf[idx-i,,],1,mean) * cfs_taf,col=clr,lwd=3,lty=2)
    if(show_legend==T){
      legend('topright',c('Obs',labs[1],labs[2]),
           col=c('black','light gray',clr),lwd=c(3,1,3),lty=c(1,1,2),cex=2)}
  }
  if(plot_median==T){
    lines(0:14,apply(synft[idx-i,,],1,median) * cfs_taf,col=clr,lwd=3,lty=3)
    if(show_legend==T){
      legend('topright',c('Obs',labs[1],labs[3]),
          col=c('black','light gray',clr),lwd=c(3,1,3),lty=c(1,1,2),cex=2)}
  }
  if(plot_samp==T){
    lines(0:14,synf[(idx-i),,synf_samp] * cfs_taf,col='magenta')
  }
  lines(0:14,synf[(idx-i):(idx+14-i),1,1] * cfs_taf,lwd=3,col='black')
  ...
  if(show_yaxslab==T){
    axis(2,at=seq(0,ylm,5),seq(0,ylm,5))}
}
