...
new_obs<-inf[which(inf$GMT=='10/1/1948 12:00'):which(inf$GMT=='9/30/1985 12:00'),4:6]
new_obs[new_obs<0]<-0
...
ix3<-seq(as.Date('1985-10-15'),as.Date('2010-09-30'),'day')
ixx3<-as.POSIXlt(ix3)

ix<-seq(as.Date('1948-10-01'),as.Date('1985-09-30'),'day')
ix2<-as.POSIXlt(ix)

loess_fit<-readRDS('data/fit/loess_fit_loessv3-1_hopper_12z.rds')
loess_mat<-array(NA,dim(new_obs_mat))

for(i in 1:12){
  seas<-which(ix2$mon==(i-1))
  #seas2<-which(ixx3$mon==(i-1))
  for(k in 1:3){
    obs_inf<-matrix(rep(new_obs[seas,k],leads),ncol=leads,byrow=F)
    loess_inf<-array(NA,dim(obs_inf))
    #hc_ob<-obs[seas2,k]
    for(j in 1:leads){
      ob_pred<-obs_inf[,j]
      cmn<-predict(loess_fit[[j]][[i]][[k]],ob_pred)
      #hi_cm_idx<-which.max(cmn)
      #ob_flr<-ob_pred[hi_cm_idx]
      #inp<-which(ob_pred>ob_flr)
      #repl<-cmn[inp]
      #repl[repl<cmn[hi_cm_idx]]<-cmn[hi_cm_idx]
      #cmn[inp]<-repl
      zero_idx<-which(cmn<0)
      na_idx<-which(is.na(cmn)==T)
      cmn[zero_idx]<-ob_pred[zero_idx]
      cmn[na_idx]<-ob_pred[na_idx]
      loess_inf[,j]<-cmn
    }
...
#for(i in 1:14){
  #for(j in 1:12){
    #seas<-which(ix2$mo==(j-1))
    #seas2<-which(ixx3$mo==(j-1))
    #ob<-new_obs[seas,2]
    #cm<-loess_mat[seas,(14+i)]
    #ob2<-obs[seas2,2]
    #cm2<-cmean_mat[seas2,(14+i)]
    #plot(ob2,cm2,col='red')
    #points(ob,cm,col='black')
  #}
#}
...
yr_seq<-c(rep(48,3),rep(49:84,each=12),rep(85,9))
mo_seq<-c(10:12,rep(1:12,length(49:84)),1:9)
...
syn_hefs_flow<-array(NA,c(leads*3,length(ix2),ens_num))
...
#KNN process to generate array of 'ens_num' samples
  ...
  for(i in 1:length(yr_seq)){
    ...
    seas<-which(ix2$mon==(mo_seq[i]-1))
    ob<-loess_mat[seas3,]
    aob_seas<-new_obs_mat[seas,]
    max_vec<-apply(aob_seas,2,max)
    act_obs<-new_obs_mat[seas3,]
    ...
    for(j in 1:(leads*3)){
      ...
      syn_hefs_flow[j,seas3,e]<-loess_mat[seas3,j] - syn_resid_mat[(ar+1):k,j]
    }
...
saveRDS(synflow_out, paste('output/v10_sumrsamp30/syn_hefs_flow_4885_v10_sumrsamp30_ens_',z,'_12z.rds',sep=''))
