...
outfile_hefs<-'corr_v10-2_sumrsamp30'
...
if(dir.exists(paste('~/syn_forecast_test/output/',outfile_hefs,'/npz',sep=''))==T){
  unlink(paste('~/syn_forecast_test/output/',outfile_hefs,'/npz',sep=''),recursive = T)}
dir.create(paste('~/syn_forecast_test/output/',outfile_hefs,'/npz',sep=''))

foreach(m = 1:nsets,.packages='feather') %dopar% {
  syn_forc_in<-readRDS(paste('~/syn_forecast_test/output/',outfile_hefs,'/syn_hefs_flow_8510_',outfile_hefs,'_ens_',m,'_12z.rds',sep=''))
  ...
  for(n in 1:nsamps){
    ...
    if(dir.exists(paste('~/syn_forecast_test/output/',outfile_hefs,'/feather/samp',samp_vec[m,n],sep=''))==T){
      unlink(paste('~/syn_forecast_test/output/',outfile_hefs,'/feather/samp',samp_vec[m,n],sep=''),recursive = T)}
    dir.create(paste('~/syn_forecast_test/output/',outfile_hefs,'/feather/samp',samp_vec[m,n],sep=''))
    ...
    for(e in 1:ens_num){
      ...
      write_feather(syn_hop_fthr, paste('~/syn_forecast_test/output/',outfile_hefs,'/feather/samp',samp_vec[m,n],'/syn-hop_',e,'.feather',sep=''))
      write_feather(syn_lam_fthr, paste('~/syn_forecast_test/output/',outfile_hefs,'/feather/samp',samp_vec[m,n],'/syn-lam_',e,'.feather',sep=''))
      write_feather(syn_uka_fthr, paste('~/syn_forecast_test/output/',outfile_hefs,'/feather/samp',samp_vec[m,n],'/syn-uka_',e,'.feather',sep=''))
    }
  }
}
